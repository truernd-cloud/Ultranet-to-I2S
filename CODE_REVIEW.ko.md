...
버전 2.0(두 스트림 수신, 16채널 믹스, PWM 제거)에서 아래 항목이 바뀌었습니다. 나머지는 여전히 해당합니다.

| 항목 | 상태 |
|---|---|
| 1. 파일 이름 대소문자 | 수정됨 (`ultranet.c`) |
| 8. 공식 Pico에서 쓸 수 없는 핀 | GP29(PWM)는 더 이상 쓰지 않음. GP24(MCLK)는 그대로 |
| 11. `jmp pin`에 상수 SM 사용 | 수정됨 (`sm_config_set_jmp_pin()`으로 설정) |
| 12. `core1_entry()` 정리 | 해당 코드(셀렉터, PWM 변환)가 없어짐 |
| 13. 스테레오 쌍 프레임 불일치 | 믹스에서도 같음: 믹스 하나에 서로 다른 프레임의 채널이 섞일 수 있음 (최대 1샘플 차이, 들리지 않는 수준) |
| 15. 오래된 주석 | PIO 파일의 클럭 설명 일부 수정됨 |
| 16. 반복 코드 | 수신은 스트림별 상태 구조체로, 출력은 믹스 루프로 바뀌어 해당 없음 |
